...
sinonimos1= ["vasto","calumnia","baladi","perfidia","hosco","ofuscar","disyuntiva","diafano","dorso","pusilanime"]
...
def mainSynonym():
    ...
    lcd_init()
    lcd_string("  -Are they... ",LCD_LINE_1)
    lcd_string("    synonyms?-",LCD_LINE_2)
    time.sleep(5)
    cont=0
    Vcont=0
    ...
    while cont<5:
        ...
        if respuesta== respuestaUsuario:
            ...
            lcd_string("  (n o n)/",LCD_LINE_2)
            Vcont+=1
            time.sleep(3)
        else:
            lcd_string(" -INCORRECT-",LCD_LINE_1)
            lcd_string("  (T n T)",LCD_LINE_2)
            time.sleep(3)

    lcd_string("RIGHT ANSWERS:",LCD_LINE_1)
