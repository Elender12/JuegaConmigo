...
#Makes a file and saves the full path
f = open('naxito_database.txt',"a+")
file = os.path.realpath(f.name)
f = open(file,"a+")

#Writes data
def saveData(contComer,contDormir,contJugar):
    #f = open('naxito_database.txt',"w")
    #file = os.path.realpath(f.name)
    f = open(file,"w")
    f.write(str(contComer))
    f.write("\n")
    f.write(str(contDormir))
    f.write("\n")
    f.write(str(contJugar))
    f.close()
...
def main():
    ...
    if os.path.getsize(file) > 0:
        f = open(file,"r")
        contComer = int(f.readline())
        contDormir = int(f.readline())
        contJugar = int(f.readline())
    #file does
    else:
        contComer=0
        contDormir=0
...
def naxitosLife(contComer,contDormir,contJugar):
    saveData(contComer,contDormir,contJugar)
    ...
    if  command == "DORMIR":
        ...
        if contJugar>=1:
            ...
            lcd_string("  (_ - _)zzzZ",LCD_LINE_1)
